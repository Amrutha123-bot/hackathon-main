...
class VectorService:
    ...
    def __init__(self, supabase):
        ...
        self.chunk_repository = ChunkRepository(supabase)
    ...
    def add_chunks(
        ...
        for chunk, embedding in zip(chunks, embeddings):
            ...
            if not source:
                raise ValueError(
                    "Chunk is missing source information."
                )

            filename = os.path.basename(source)
            if not filename:
                raise ValueError(
                    "Chunk is missing source filename."
                )
            
            document_id = document_ids_by_filename.get(filename)

            if not document_id:
                raise ValueError(
                    f"No document ID found for file: {filename}"
                )
        ...
        saved_chunks = self.chunk_repository.add_chunks(rows)
    ...
    def search(
        ...
        match_count = top_k or TOP_K

        results = self.chunk_repository.search_similar_chunks(
